...
public class PolyhSignalValue implements DatasetXY {
	...
	public XYDataset createDataset(int freq) {
		...
		XYSeriesCollection dataset = new XYSeriesCollection();
			...
			XYSeries seriesRestoredSignal = new XYSeries("RestoredSignal");
			XYSeries seriesRestoredNfSignal = new XYSeries("RestoredNfSignal");

			for (int i = 0; i < N; i++) {
				seriesSignal.add(i, signal.SourceSignal[i]);
				seriesRestoredSignal.add(i, signal.RestoredSignal[i]);
				seriesRestoredNfSignal.add(i, signal.RestoredSignalWithoutPhase[i]);
			}
			dataset.addSeries(seriesSignal);
			dataset.addSeries(seriesRestoredSignal);
			dataset.addSeries(seriesRestoredNfSignal);
